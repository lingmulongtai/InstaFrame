...
- 許可は「このブラウザのセッション中」または「この端末で常に許可」から選択できます。
...
| AVI / MKV / 3GP | 条件付き | コンテナと内部コーデックの両方をブラウザが再生できる場合のみ。4ブラウザCIで入力候補として受理され、無効な内容は項目ごとの明示エラーになることを検証。成功経路は未検証 |
...
写真の出力はJPEG・PNG・WebPです。Blobの申告MIMEだけに依存せず実ファイル署名を検査し、署名が要求形式と一致する場合だけ、その拡張子で保存します。署名が正しくBlob MIMEだけが誤っている場合は、安全なMIMEへ補正します。動画出力は`MediaRecorder.isTypeSupported()`とCanvas stream captureの両方を満たすMP4またはWebM（VP8 / VP9 / ブラウザ選択）だけを表示し、完全な生成経路がないブラウザでは動画出力を無効化します。入力音声はブラウザが取得できる場合に出力へ再エンコードして保持します。MOV・M4V・AVI・MKV・3GPは入力候補であり、同じコンテナでの出力を意味しません。
...
- 写真CanvasのJPEG / PNG / WebPエンコードが60秒応答しない場合は終了し、書き出しUIを復帰
...
- EXIF編集と設定保存
- 複数JPEGのZIP出力
- 大量バッチのメモリ警告、入力・画素・Canvas・生成済み出力の安全上限、受理済みファイルの読込完了待ち、書き出しキャンセル
- EXIF・ZIP・フォントが必要になるまで初回取得されないこと
- 処理中エンコーダーと写真エンコードへのAbortSignal伝播、キャンセル後の同一Canvas再エンコード直列化、非表示タブでのMediaRecorder・動画ソース同時停止と復帰、WebCodecsの背圧・生成途中の出力上限、動画メタデータ・フレーム進行のタイムアウト
...
- `captureStream`、`mozCaptureStream`、Web Audio fallbackから出力streamへ音声trackを渡す決定的な4ブラウザ契約（実コーデックでの音声保持証拠とは区別）
...
IDE設定、テスト、Node.js依存関係、READMEなどは公開アーティファクトに含めません。Pull Requestと`main`へのpushでは、CIが構文・Lint・ユニット検査を1回実行し、Pagesと同じ `dist/` を生成してから、ChromiumのフルE2EとFirefox・WebKit・Microsoft Edgeの写真/UI/axe/プライバシー/音声ルーティング契約をマトリクス実行します。各ブラウザはローカル資産のrevisionと、リポジトリ内部ファイルが配信されないことも確認します。動画エンコード、HEIC/HEIF、MP4/MOVはブラウザ・OSコーデック依存のため、対応を一律には保証しません。
