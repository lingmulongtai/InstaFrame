...
- 5120px固定レイアウト、最大800%ズーム、ズーム追従の高密度ライブプレビュー
...
3. Autoは通常表示でも最低2倍、ズーム時は倍率に合わせて最大8倍のバックバッファへ再描画
4. プレビュー用の縮小画素はJPEGへ再圧縮せず、ロスレスなCanvasのままフレームへ渡す

そのため画質変更や最大800%までのズームはシャープさと表示範囲だけに影響し、アスペクト比、余白、文字位置には影響しません。ライブCanvasはデスクトップ24メガピクセル、モバイル12メガピクセルを上限として、端末メモリを使い切らない範囲で倍率へ追従します。
